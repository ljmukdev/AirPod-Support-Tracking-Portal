...
async function loadProducts() {
    ...
    try {
        ...
        if (response.ok && data.products) {
            // Pass products to filter system - it handles all rendering
            if (typeof setProductsForFiltering === 'function') {
                setProductsForFiltering(data.products);
            } else {
                // Fallback if filter system not loaded
                tableBody.innerHTML = '<tr><td colspan="8" style="text-align: center; padding: 20px;">Filter system not loaded</td></tr>';
            }
        } else {
